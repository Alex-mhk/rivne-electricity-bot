...
# Токен вашого Telegram бота (з змінних оточення)
BOT_TOKEN = os.getenv("BOT_TOKEN")

if not BOT_TOKEN:
    logger.error("BOT_TOKEN не встановлений! Додай змінну на Render.")
    exit(1)
    ...
    async def fetch_schedule():
        ...
        try:
            async with aiohttp.ClientSession() as session:
                async with session.get(RivneElectricityParser.SITE_URL, timeout=aiohttp.ClientTimeout(total=10)) as response:
            ...
            if not table:
                logger.warning("Таблиця не знайдена на сайті")
                return None
            ...
            for row in rows:
                ...
                if len(cells) > 0:
                    ...
                    if cell_text == today or cell_text == tomorrow:
                        # Черга 6.2 знаходиться в позиції 11
                        if len(cells) >= 12:
...
async def main():
    ...
    logger.info(f"Токен отриманий: {BOT_TOKEN[:10]}...")
